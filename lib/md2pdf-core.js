...
function _parseFmValue(fm, key) {
  ...
  const quotedMatch = fm.match(new RegExp(`^${key}:\\s*["'](.+?)["']\\s*$`, "m"));
...
async function md2pdfTh(options = {}) {
  ...
  // 3. Cover page (use frontmatter.title if available)
  if (cover) {
  ...
  const showHeader = !noPageNumbers || headerText;
  const showFooter = !noPageNumbers || footerText;
  const headerTemplate = showHeader ? (headerText ? `<div style="font-size:8px;width:100%;text-align:center;color:#94a3b8">${escapeHtml(headerText)}</div>` : PAGE_HEADER_TEMPLATE) : "";
  const footerTemplate = showFooter ? (footerText ? `<div style="font-size:9px;width:100%;text-align:center;color:#94a3b8">${escapeHtml(footerText)}</div>` : PAGE_FOOTER_TEMPLATE) : "";
  ...
  const pdf = await mdToPdf({ content: finalContent }, {
    ...
    pdf_options: { format: format || "A4", margin: marginConfig, printBackground: true, displayHeaderFooter, headerTemplate, footerTemplate, preferCSSPageSize: false },
