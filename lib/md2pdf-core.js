...
function parseFrontmatter(content) {
  ...
  if (match) {
    const fm = match[1];
    const t = fm.match(/^title:\s*(.+)$/m); if (t) meta.title = t[1].trim().replace(/['"]/g, "");
    const a = fm.match(/^author:\s*(.+)$/m); if (a) meta.author = a[1].trim().replace(/['"]/g, "");
    const d = fm.match(/^date:\s*(.+)$/m); if (d) meta.date = d[1].trim().replace(/['"]/g, "");
    const tg = fm.match(/^tags:\s*(.+)$/m); if (tg) meta.tags = tg[1].split(",").map(s => s.trim().replace(/['"]/g, ""));
    const desc = fm.match(/^description:\s*(.+)$/m); if (desc) meta.description = desc[1].trim().replace(/['"]/g, "");
