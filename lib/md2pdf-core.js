 ...
 * @param {string} [options.format='A4'] - Page size (metadata only)
 ...
 * @returns {Promise<{html: string, css: string, title: string, headerTitle: string, frontmatter: Object, baseName: string}>} HTML output
 */
async function md2html(options = {}) {
  const {
    content, inputPath, css: customCss, cssPath,
    theme = "light", toc = false, cover = false,
    headerText, footerText, font,
  ...
  const html = `<!DOCTYPE html><html lang="${fmLang || 'th'}"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>${escapeHtml(headerTitle)}</title><style>${css}</style></head><body>${marked.parse(finalContent)}</body></html>`;

  return { html, css, title, headerTitle, frontmatter, baseName };
}
 ...
 * @param {number} [options.timeout] - Conversion timeout in milliseconds
...
async function md2pdfTh(options = {}) {
  const {
    ...
    noPageNumbers = false, lang = "th",
    template, watermark, outputFilename, timeout,
  } = options;

  // Re-use HTML generation
  const { html, frontmatter, headerTitle, baseName } = await md2html({
    content, inputPath, css: customCss, cssPath,
    theme, toc, cover, headerText, footerText, font,
    noPageNumbers, lang, template,
  });

  // Parse the HTML body content back out for md-to-pdf
  const bodyMatch = html.match(/<body>([\s\S]*)<\/body>/);
  const finalContent = bodyMatch ? bodyMatch[1] : html;

  // Extract CSS from html
  const cssMatch = html.match(/<style>([\s\S]*?)<\/style>/);
  const css = cssMatch ? cssMatch[1] : "";

  // Merge frontmatter options
  const ek = frontmatter._explicitKeys;
  const fmTheme = ek.has("theme") ? frontmatter.theme : theme;
  ...
  // Puppeteer
  const launchArgs = ["--font-render-hinting=medium", "--disable-dev-shm-usage"];
  if (process.env.CI) launchArgs.push("--no-sandbox", "--disable-setuid-sandbox");

  // Convert
  let pdf;
  try {
    const conversion = mdToPdf({ content: finalContent }, {
      css,
    ...
    if (timeout && timeout > 0) {
      const timeoutPromise = new Promise((_, reject) =>
        setTimeout(() => reject(new Error(`Conversion timed out after ${timeout}ms`)), timeout)
      );
      pdf = await Promise.race([conversion, timeoutPromise]);
    } else {
      pdf = await conversion;
    }
  } catch (err) {
